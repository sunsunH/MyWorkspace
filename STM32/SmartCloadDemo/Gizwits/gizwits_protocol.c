...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagSmartSwith)
    {
        info->event[info->num] = EVENT_SmartSwith;
        info->num++;
        dataPoints->valueSmartSwith = gizStandardDecompressionValue(SmartSwith_BYTEOFFSET,SmartSwith_BITOFFSET,SmartSwith_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueSmartSwith != cur->valueSmartSwith)
    {
        GIZWITS_LOG("valueSmartSwith Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(SmartSwith_BYTEOFFSET,SmartSwith_BITOFFSET,SmartSwith_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSmartSwith);
